...
async def areq(url: str, headers: dict, params: dict, func):
...
class WASMProcessor:
    def __init__(self, embed_url: str) -> None:
        ...
        self.store: Store = Store(self.engine)

        self.wasm: Instance
        self.wasm_memory: Memory

        self.arr: list[Any] = [None] * 128
        ...
        self.canvas.baseUrl = embed_url
        self.node_list.image.src = f"{base_url}/images/image.png?v=0.1.0"
        self.image_data.data = self.get_pixel_arr(self.node_list.image.src)

        self.fake_window.location.href = embed_url
        self.fake_window.xrax = xrax
    ...
    def write_to_memory(self, data, offset: int) -> None:
        ...
        self.wasm_memory.write(self.store, buffer)

    def set_float64(self, offset: int, value: int) -> None:
        self.write_to_memory(struct.pack("<d", value), offset)

    def set_int32(self, offset: int, value: int) -> None:
        self.write_to_memory(struct.pack("<i", value), offset)

    def get_pixel_arr(self, url: str) -> list[int]:
        data = requests.get(url).content

        with PIL.Image.open(BytesIO(data)) as image:
            arr = array(image).flatten().tolist()
    ...
    def parse(self, text: str) -> int:
        ...
        buffer = bytearray()
        text_len = len(text)
        offset = self.export0(text_len, 1)
        ...
        self.write_to_memory(buffer, offset)

        return offset

    def get_memory_buffer(self) -> bytearray:
        return self.wasm_memory.read(self.store)

    def shift(self, index: int) -> None:
        if index >= 132:
            self.arr[index] = self.pointer
            self.pointer = index

    def shift_get(self, index: int):
        item = self.get(index)
        self.shift(index)

        return item

    def decode_sub(self, index: int, offset: int) -> str:
        index &= 0xFFFFFFFF

        return self.get_memory_buffer()[index : index + offset].decode()

    def add_to_stack(self, obj) -> int:

        if self.pointer == len(self.arr):
            self.arr.append(len(self.arr) + 1)

        Qn = self.pointer
        self.pointer = self.get(Qn)
        self.arr[Qn] = obj

        return Qn

    def args(self, QP, Qn, QT, func):
        Qx = {
            "a": QP,
            "b": Qn,
            "cnt": 1,
        ...
        __wbindgen_export_2 = self.wasm.exports(self.store)["__wbindgen_export_2"]
    ...
    def getfunc(self, name: str):
        return self.wasm.exports(self.store)[name]

    def export0(self, Qp, Qn) -> int:
        __wbindgen_export_0 = self.getfunc("__wbindgen_export_0")
        return __wbindgen_export_0(self.store, Qp, Qn) & 0xFFFFFFFF  # type: ignore

    def export3(self, Qp, Qn) -> int:
        __wbindgen_export_3 = self.getfunc("__wbindgen_export_3")
        return self.shift_get(__wbindgen_export_3(self.store, Qp, Qn))  # type: ignore

    def export4(self, Qy, QO, Qx):
        __wbindgen_export_4 = self.getfunc("__wbindgen_export_4")
        __wbindgen_export_4(self.store, Qy, QO, self.add_to_stack(Qx))  # type: ignore

    def export5(self, Qp, Qn):
        __wbindgen_export_4 = self.getfunc("__wbindgen_export_5")
        __wbindgen_export_4(self.store, Qp, Qn)  # type: ignore

    def write_png(self, decoded_png):

        png_len = len(decoded_png)
        self.size = png_len

        offset = self.export0(png_len, 1)
        self.write_to_memory(bytearray(decoded_png), offset)

        return offset

    def load_wasm_imports(self) -> Linker:
        linker = Linker(self.engine)

        def add_import_func(type: FuncType, func, name: str | None = None):
            linker.define(self.store, "wbg", func.__name__ if not name else name, Func(self.store, type, func))
        ...
        def __wbindgen_string_new(index, offset):
            string = self.decode_sub(index, offset)
            return self.add_to_stack(string)

        add_import_func(TYPE0, __wbindgen_string_new)

        def __wbindgen_boolean_get(index):
            boolean = self.get(index)
        ...
        def __wbindgen_number_get(offset, index):
            number = self.get(index)

            self.set_float64(offset + 8, 0 if self.is_null(number) else number)
            self.set_int32(offset, 0 if self.is_null(number) else 1)
        ...
        def __wbg_getImageData_66269d289f37d3c7(*_):
            return self.add_to_stack(self.image_data)
        ...
        def __wbg_toDataURL_97b108dd1a4b7454(offset, _):
            data_url_ = self.parse(data_url)

            self.set_int32(offset + 4, self.size)
            self.set_int32(offset, data_url_)
        ...
        def __wbg_src_87a0e38af6229364(offset, index):
            ...
            src = self.parse(image.src)

            self.set_int32(offset + 4, self.size)
            self.set_int32(offset, src)
        ...
        def __wbg_data_d34dc554f90b8652(offset, index):
            imagedata = self.get(index)
            data = self.write_png(imagedata.data)

            self.set_int32(offset + 4, self.size)
            self.set_int32(offset, data)

        add_import_func(TYPE2, __wbg_data_d34dc554f90b8652)

        def __wbg_origin_305402044aa148ce(offset, index):
            location = self.get(index)
            origin = self.parse(location.origin)

            self.set_int32(offset + 4, self.size)
            self.set_int32(offset, origin)

        add_import_func(TYPE2, __wbg_origin_305402044aa148ce)

        def __wbg_length_8a9352f7b7360c37(index):
            nodelist = self.get(index)
        ...
        def __wbg_get_8986951b1ee310e0(index, decode_index, decode_offset):
            fakewindow = self.get(index)
            attr = self.decode_sub(decode_index, decode_offset)

            data = getattr(fakewindow, attr)
            res = 0 if self.is_null(data) else self.add_to_stack(data)
        ...
        def __wbg_msCrypto_eb05e62b530a1508(index):
            fakewindow = self.get(index)
            return self.add_to_stack(fakewindow.msCrypto)
        ...
        def __wbg_self_05040bd9523805b9():
            return self.add_to_stack(self.fake_window)
        ...
        def __wbg_eval_c824e170787ad184(index, offset):
            fake_str = f"fake_{self.decode_sub(index, offset)}"

            try:
                if "fake_window.pid" in fake_str:
                    pid = fake_str.split("=")[1].replace("'", "").strip()
                    self.fake_window.pid = pid

                    eval_result = pid

                else:
                    eval_result = eval("self." + fake_str)

            except AttributeError:
                eval_result = None

            return self.add_to_stack(eval_result)
        ...
        def __wbg_buffer_b914fb8b50ebbc3e(index):
            memory = self.get(index)
            return self.add_to_stack(memory.read(self.store))
        ...
        def __wbg_subarray_adc418253d76e2f1(index, start, end):
            ...
            buffer = self.get(index)
            return self.add_to_stack(bytearray(buffer[start:end]))
    ...
    def assign_wasm(self, wasm_instance: Instance) -> None:
        self.wasm = wasm_instance
        self.wasm_memory = self.wasm.exports(self.store)["memory"]  # type: ignore

    def create_wasm_instance(self, wasm: str | bytes, imports: Linker) -> Instance:
        module = Module(self.engine, wasm)
        instance = imports.instantiate(self.store, module)
    ...
    async def load_wasm(self, url: str) -> bytearray:
        imports = self.load_wasm_imports()
        headers = {
            "Referer": self.fake_window.location.href,
            "Host": "megacloud.club",
        }
        b_resp = await areq(url, headers, {"v": "0.0.9"}, lambda i: i.read())
        instance = self.create_wasm_instance(b_resp, imports)

        self.assign_wasm(instance)

        return bytearray(instance.bytes)  # type: ignore

    async def run_wasm(self) -> bytes:
        await self.set_meta(self.embed_url)

        wasm_bytes = await self.load_wasm(f"{base_url}/images/loading.png")
        setattr(self.fake_window, "bytes", wasm_bytes)

        self.wasm.exports(self.store)["groot"](self.store)  # type: ignore
        self.fake_window.jwt_plugin(wasm_bytes)
        result = self.fake_window.navigate()

        return result

    async def set_meta(self, url) -> None:
        headers = {
            "User-Agent": user_agent,
            "Referer": "https://hianime.to/",
        }

        html_resp = await areq(url, headers, {}, lambda i: i.text())
        pattern = r"name=\"j_crt\" content=\"([\w=]+)"

        if not (match := re.search(pattern, html_resp)):
            raise ValueError("no key found")

        self.meta.content = match.group(1)
...
async def extract(embed_url: str) -> dict:
    ...
    params = {
        "id": wasm.fake_window.pid,
        "v": wasm.fake_window.localStorage.kversion,
        "h": wasm.fake_window.localStorage.kid,
        "b": wasm.fake_window.browser_version,
    }

    resp = await areq(src_url, headers, params, lambda i: i.json())

    if not resp["sources"]:
        raise ValueError("no sources found")

    # decrypt
    q3 = int(wasm.fake_window.localStorage.kversion)
    q1 = split_int32(q3)
    q5 = bytearray(q5)
    q8: bytearray
